...
final MethodChannel _channel = const MethodChannel('com.jarvanmo/rammus')..setMethodCallHandler(_handler);
...
///callback 回调
Future<CommonCallbackResult> bindTag({@required CloudPushServiceTarget target, List<String> tags, String alias}) async {
  var result = await _channel
      .invokeMethod("bindTag", {"target": target.index + 1, "tags": tags ?? List<String>.from([]), "alias": alias});
...
Future<CommonCallbackResult> unbindTag(
  ...
  var result = await _channel
      .invokeMethod("unbindTag", {"target": target.index + 1, "tags": tags ?? List<String>.from([]), "alias": alias});
...
Future configureNotificationPresentationOption(
  ...
  return _channel.invokeMethod(
      "configureNotificationPresentationOption", {"none": none, "sound": sound, "alert": alert, "badge": badge});
}
...
Future<dynamic> _handler(MethodCall methodCall) {
  if ("initCloudChannelResult" == methodCall.method) {
    ...
    _onNotificationController.add(OnNotification(
        methodCall.arguments["title"], methodCall.arguments["summary"], jsonDecode(methodCall.arguments["extras"])));
    ...
    _onNotificationClickedWithNoActionController.add(OnNotificationClickedWithNoAction(
        methodCall.arguments["title"], methodCall.arguments["summary"], methodCall.arguments["extras"]));
